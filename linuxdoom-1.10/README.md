...
not able to release the DOS code, and the NeXT code was taken out too, leaving
the Linux source to be released. The Linux code was a fork from the main
codebase, however, and BK had to make some changes to it to get it workable
...
want a fully featured port supporting all the iwads etc while being fully
compatible with the original game, you should be looking at prboom.

This version is network compatible with the original doom2 using ipxsetup.exe.
There is no separate networking program for linuxdoom-1.11 - the IPX network
support is built in. This means that this version probably only compiles on
...
the UDP networking from linuxdoom-1.10 - if you want an easy networking, you
should be using one of the ports.

This isn't intended for end-users. If you want an easy to use, functional Doom
port for your system, use one of the ports. PrBoom is the best if you want
authenticity to the original game; for new features and eye-candy, you should
look at ZDoom, or one of the OpenGL ports available on doomworld. This release
is just intended for developers who might find it useful to have a true
v1.9-compatible compilable & usable source code base. And for people like me,
